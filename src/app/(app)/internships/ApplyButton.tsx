...
export default function ApplyButton({ listingId }: { listingId: string }) {
  ...
  return (
    <form action={action} className="space-y-2">
      ...
      <button
        type="submit"
        disabled={pending}
        className="flex h-9 w-full items-center justify-center gap-2 rounded-xl bg-indigo-600 text-xs font-semibold text-white hover:bg-indigo-700 disabled:opacity-60"
      >
        {pending && <Loader2 className="size-3.5 animate-spin" />} Apply now
      </button>
